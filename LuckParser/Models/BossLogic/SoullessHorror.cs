            ...
            switch (boss.ID)
            {
                case (ushort)ParseEnum.BossIDS.SoullessHorror:
                    ...
                    foreach (CastLog c in deathBloom)
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            replay.Actors.Add(new PieActor(true, 0, 3500, getRotationFromFacing(facing) + (i * 360 / 8), 360 / 12, new Tuple<int, int>(start, end), "rgba(255,200,0,0.5)", new AgentConnector(boss)));
                        }
                    ...
                    foreach (CastLog c in quad1)
                    {
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            replay.Actors.Add(new PieActor(true, 0, 3500, getRotationFromFacing(facing) + (i * 360 / 4), 360 / 12, new Tuple<int, int>(start, end), "rgba(255,200,0,0.5)", new AgentConnector(boss)));
                        }
                    ...
                    foreach (CastLog c in quad2)
                    {
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            replay.Actors.Add(new PieActor(true, 0, 3500, getRotationFromFacing(facing) + 45 + (i * 360 / 4), 360 / 12, new Tuple<int, int>(start, end), "rgba(255,200,0,0.5)", new AgentConnector(boss)));
                        }

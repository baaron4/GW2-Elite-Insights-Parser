    ...
    public class SoullessHorror : RaidLogic
    {
        ...
        public override List<ParseEnum.TrashIDS> GetAdditionalData(CombatReplay replay, List<CastLog> cls, ParsedLog log)
        {
            ...
            foreach (CastLog c in deathBloom)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    replay.AddCircleSegmentActor(new CircleSegmentActor(true, 0, 3500, (int)(Math.Atan2(-facing.Y, facing.X) * 180 / Math.PI + i * 360 / 8), 360 / 12, new Tuple<int, int>(start, end), "rgba(255,200,0,0.5)"));
                }
            ...
            foreach (CastLog c in quad1)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    replay.AddCircleSegmentActor(new CircleSegmentActor(true, 0, 3500, (int)(Math.Atan2(-facing.Y, facing.X) * 180 / Math.PI + i * 360 / 4), 360 / 12, new Tuple<int, int>(start, end), "rgba(255,200,0,0.5)"));
                }
            ...
            foreach (CastLog c in quad2)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    replay.AddCircleSegmentActor(new CircleSegmentActor(true, 0, 3500, (int)(Math.Atan2(-facing.Y, facing.X) * 180 / Math.PI + 45 + i * 360 / 4), 360 / 12, new Tuple<int, int>(start, end), "rgba(255,200,0,0.5)"));
                }

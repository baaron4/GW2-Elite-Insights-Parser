...
namespace LuckParser.Models
{
    public class Samarog : RaidLogic
    {
        public Samarog()
        {
            ...
            new Mechanic(38013, "Brutalize", Mechanic.MechType.SkillOnPlayer, ParseEnum.BossIDS.Samarog, "symbol:'diamond-tall',color:'rgb(255,0,0)',", "CC.Fail","Brutalize (Failed CC)", "CC Fail",0,(condition => condition.getDLog().GetResult() == ParseEnum.Result.KillingBlow)),
            new Mechanic(38013, "Brutalize", Mechanic.MechType.EnemyCastEnd, ParseEnum.BossIDS.Samarog, "symbol:'diamond-tall',color:'rgb(0,160,0)',", "CC.End","Ended Brutalize", "CC Ended",0,(condition => (condition.getCombatItem().IsActivation == ParseEnum.Activation.CancelCancel || condition.getCombatItem().IsActivation == ParseEnum.Activation.CancelFire))),
        ...
        public override List<PhaseData> GetPhases(Boss boss, ParsedLog log, List<CastLog> castLogs)
        {
            ...
            for (int i = 1; i < phases.Count; i++)
            {
                ...
                if (i == 2 || i == 4)
                {
                    ...
                    List<AgentItem> slaves = log.AgentData.GetNPCAgentList().Where(x => ids.Contains(ParseEnum.GetThrashIDS(x.ID))).ToList();

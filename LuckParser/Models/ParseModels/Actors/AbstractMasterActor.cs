                    ...
                    BoonsGraphModel graph = BoonPoints[boonid]; switch (boonid)
                    {
                        ...
                        case 50421:
                            foreach (Target target in log.FightData.Logic.Targets)
                            {
                                ...
                                if (!dict.TryGetValue(boonid, out var list))
                                {
                                    ...
                                    for (int i = 0; i < phases.Count; i++)
                                    {
                                        ...
                                        extraDataList.Add(new ExtraBoonData(effect.Count, dmLogs.Count(x => !x.IsIndirectDamage), damage, totalDamage));
                                    }
                            ...
                            for (int i = 0; i < phases.Count; i++)
                            {
                                ...
                                List<DamageLog> effect = dmLogs.Where(x => graph.GetStackCount(x.Time) > 0 && !x.IsIndirectDamage).ToList();
                                int damage = (int)Math.Round(effect.Sum(x => x.Damage) / 21.0);
                                _boonExtra[boonid].Add(new ExtraBoonData(effect.Count, dmLogs.Count(x => !x.IsIndirectDamage), damage, totalDamage));
                            }
                            break;
                        // GoE
                        case 31803:
                            foreach (Target target in log.FightData.Logic.Targets)
                            {
                                ...
                                if (!dict.TryGetValue(boonid, out var list))
                                {
                                    ...
                                    for (int i = 0; i < phases.Count; i++)
                                    {
                                        ...
                                        extraDataList.Add(new ExtraBoonData(effect.Count, dmLogs.Count(x => !x.IsIndirectDamage), damage, 0));
                                    }
                                    dict[boonid] = extraDataList;
                                }
                            ...
                            for (int i = 0; i < phases.Count; i++)
                            {
                                ...
                                List<DamageLog> effect = dmLogs.Where(x => graph.GetStackCount(x.Time) > 0 && !x.IsIndirectDamage).ToList();
                                int damage = effect.Sum(x => x.Damage);
                                _boonExtra[boonid].Add(new ExtraBoonData(effect.Count, dmLogs.Count(x => !x.IsIndirectDamage), damage, 0));
                            }
                            break;
                    }
                }

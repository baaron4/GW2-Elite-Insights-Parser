...
namespace LuckParser.Models.Logic
{
    public class Arkk : FractalLogic
    {
        public Arkk(ushort triggerID) : base(triggerID)
        {
            ...
            new PlayerBoonApplyMechanic(791, "Fear", new MechanicPlotlySetting("square-open","rgb(255,0,0)"), "Eye","Hit by the Overhead Eye Fear", "Eye (Fear)",0, new List<BoonApplyMechanic.BoonApplyChecker>{ (ba, log) => ba.AppliedDuration == 3000 }), // //not triggered under stab, still get blinded/damaged, seperate tracking desired?
            ...
            new EnemyCastEndMechanic(39645, "Breakbar End", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "CC.Fail","Breakbar (Failed CC)", "CC Fail",0, new List<CastMechanic.CastChecker>{ (ce,log) => ce.ActualDuration > 9668}),
            new EnemyCastEndMechanic(39645, "Breakbar End", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "CCed","Breakbar broken", "CCed",0, new List<CastMechanic.CastChecker>{ (ce, log) => ce.ActualDuration < 9668 }),
            ...
            new EnemyCastEndMechanic(35761, "Pull Charge", new MechanicPlotlySetting("bowtie","rgb(255,0,0)"), "Pull CC Fail","Pull Charge CC failed", "CC fail (Gladiator)",0, new List<CastMechanic.CastChecker>{ (ce,log) => ce.ActualDuration > 3200 }), //
            new EnemyCastEndMechanic(35761, "Pull Charge", new MechanicPlotlySetting("bowtie","rgb(0,160,0)"), "Pull CCed","Pull Charge CCed", "CCed (Gladiator)",0, new List<CastMechanic.CastChecker>{ (ce, log) => ce.ActualDuration < 3200 }), //

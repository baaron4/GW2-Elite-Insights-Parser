...
namespace LuckParser.Models.Logic
{
    public class Arkk : FractalLogic
    {
        public Arkk(ushort triggerID, AgentData agentData) : base(triggerID, agentData)
        {
            ...
            new EnemyCastMechanic(39645, "Breakbar End", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "CC.Fail","Breakbar (Failed CC)", "CC Fail",0, new List<CastMechanic.CastChecker>{ (ce,log) => (ce.ReducedAnimation || ce.FullAnimation || ce.ActualDuration > 9668)}, Mechanic.TriggerRule.AND),
            new EnemyCastMechanic(39645, "Breakbar End", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "CCed","Breakbar broken", "CCed",0, new List<CastMechanic.CastChecker>{ (ce, log) => (ce.Interrupted|| ce.ActualDuration < 9668) }, Mechanic.TriggerRule.AND),
            ...
            new EnemyCastMechanic(35761, "Pull Charge", new MechanicPlotlySetting("bowtie","rgb(255,0,0)"), "Pull CC Fail","Pull Charge CC failed", "CC fail (Gladiator)",0, new List<CastMechanic.CastChecker>{ (ce,log) => (ce.ReducedAnimation || ce.FullAnimation || ce.ActualDuration > 3200) }, Mechanic.TriggerRule.AND), //
            new EnemyCastMechanic(35761, "Pull Charge", new MechanicPlotlySetting("bowtie","rgb(0,160,0)"), "Pull CCed","Pull Charge CCed", "CCed (Gladiator)",0, new List<CastMechanic.CastChecker>{ (ce, log) => (ce.Interrupted || ce.ActualDuration < 3200) }, Mechanic.TriggerRule.AND), //

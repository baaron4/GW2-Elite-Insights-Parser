...
namespace LuckParser.Models.Logic
{
    public class SoullessHorror : RaidLogic
    {
        public SoullessHorror(ushort triggerID, AgentData agentData) : base(triggerID, agentData)
        {
            ...
            new EnemyCastMechanic(48662, "Howling Death", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "CCed","Howling Death (Breakbar) broken", "CCed",0,new List<CastMechanic.CastChecker>{ (ce, log) => (ce.Interrupted || ce.ActualDuration <= 6800) }, Mechanic.TriggerRule.AND),
            new EnemyCastMechanic(48662, "Howling Death", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "CC Fail","Howling Death (Breakbar failed) ", "CC Fail",0,new List<CastMechanic.CastChecker>{ (ce,log) => (ce.ReducedAnimation || ce.FullAnimation || ce.ActualDuration > 6800) }, Mechanic.TriggerRule.AND),

...
namespace LuckParser.Models.Logic
{
    public class Matthias : RaidLogic
    {
        public Matthias(ushort triggerID) : base(triggerID)
        {
            ...
            new PlayerBoonRemoveMechanic(34442, "Sacrifice", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "CC.End","Sacrifice (Breakbar) ended", "Sacrifice End",0, new List<BoonRemoveMechanic.BoonRemoveChecker>{(br,log) => br.RemovedDuration > 25}),
            new PlayerBoonRemoveMechanic(34442, "Sacrificed", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "CC.Fail","Sacrifice time ran out", "Sacrificed",0, new List<BoonRemoveMechanic.BoonRemoveChecker>{ (br,log) => br.RemovedDuration <= 25 }),
            new PlayerBoonRemoveMechanic(34367, "Unbalanced", new MechanicPlotlySetting("square","rgb(200,140,255)"), "KD","Unbalanced (triggered Storm phase Debuff)", "Knockdown",0, new List<BoonRemoveMechanic.BoonRemoveChecker>{ (br,log) => br.RemovedDuration > 0, (de, log) => !de.To.HasBuff(log, 1122, de.Time)}),
            ...
            new PlayerBoonApplyMechanic(26766, "Icy Patch", new MechanicPlotlySetting("circle-open","rgb(0,0,255)"), "Icy KD","Knockdown by Icy Patch", "Icy Patch KD",0, new List<BoonApplyMechanic.BoonApplyChecker>{ (br,log) => br.AppliedDuration == 10000 }),

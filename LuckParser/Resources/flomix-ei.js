...
function generateWindow(layout) {
	...
	$.each(data.phases, function (i, phaseData) {
		...
		createDistTable($('#boss_dist_table' + i), data.boss.details.dmgDistributions[i], false, data.boss);
		$.each(data.boss.minions, function (m, minion) {
			createDistTable($('#boss_dist_table' + m + '_' + i), data.boss.details.minions[m].dmgDistributions[i], false, data.boss, minion);
		});
	});
...
function buildWindowLayout(data) {
	...
	$.each(data.phases, function (i, phase) {
		...
		var condiTab = {
			name: 'Boon Stats', content: [
				{ table: 'bossCondis' + i, caption: 'Condition Generation' }
			], noTitle: true
		};
		if (phase.bossHasBoons) {
			condiTab.content.push({ table: 'bossBoons' + i, caption: 'Boon Uptime' });
		}
		...
		$.each(data.boss.minions, function (m, minion) {
			bossTabs.push({ name: minion.name, content: { table: 'boss_dist_table' + m + '_' + i }, noTitle: true });
		});
		...
		var phaseTabs = [
			{
				name: 'Stats', content: {
					tabs: [
						...
						{
							name: 'Damage Stats', noTitle: true, content: {
								tabs: [
			...
			{ name: 'Damage Graph', noTitle: true, content: { tabs: dpsGraphTabs } },
			{
				name: 'Boons', content: {
					tabs: [
						{
							name: 'Boons', noTitle: true, content: {
								tabs: [
						...
						{
							name: 'Damage Buffs', noTitle: true, content: {
								tabs: [
						...
						{
							name: 'Defensive Buffs', noTitle: true, content: {
								tabs: [

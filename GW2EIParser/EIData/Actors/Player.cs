...
namespace GW2EIParser.EIData
{
    public class Player : AbstractSingleActor
    {
        ...
        public Player(AgentItem agent, bool noSquad) : base(agent)
        {
            ...
            if (name.Length < 2)
            {
                throw new InvalidOperationException("Error Encountered: Name problem on Player");
            }
            if (name[1].Length == 0 || name[2].Length == 0 || Character.Contains("-"))
            {
                throw new InvalidOperationException("Error Encountered: Missing Group on Player");
            }

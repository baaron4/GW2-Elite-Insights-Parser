...
namespace GW2EIParser.Logic
{
    public class Qadim : RaidLogic
    {
        ...
        public Qadim(ushort triggerID) : base(triggerID)
        {
            ...
            new EnemyCastEndMechanic(51943, "Qadim CC", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "Qadim CCed","Qadim Breakbar broken", "Qadim CCed",0, (ce, log) => ce.Duration < 6500),
            ...
            new EnemyCastEndMechanic(52941, "Fiery Meteor", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "Hydra CCed","Fiery Meteor (Hydra Breakbar broken)", "Hydra CCed",0,(ce, log) => ce.Duration < 12364),
            new EnemyCastEndMechanic(52941, "Fiery Meteor", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "Hydra CC Fail","Fiery Meteor (Hydra Breakbar not broken)", "Hydra CC Failed",0, (ce,log) => ce.Duration >= 12364),
            ...
            new EnemyCastEndMechanic(53132, "Patriarch CC", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "Wyv CCed","Platform Destruction (Patriarch Breakbar broken)", "Patriarch CCed",0, (ce, log) => ce.Duration < 6500),
            ...
            new EnemyCastEndMechanic(52054, "Summon", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "Dest CCed","Summon (Destroyer Breakbar broken)", "Destroyer CCed",0, (ce, log) => ce.Duration < 8332),
            new EnemyCastEndMechanic(52054, "Summon", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "Dest CC Fail","Summon (Destroyer Breakbar failed)", "Destroyer CC Fail",0, (ce,log) => ce.Duration >= 8332),
        ...
        public override void ComputeNPCCombatReplayActors(NPC target, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (target.ID)
            {
                case (ushort)ParseEnum.TargetIDS.Qadim:
                    ...
                    {
                        int radius = ccRadius;
                        replay.Decorations.Add(new CircleDecoration(true, 0, ccRadius, ((int)c.Time, (int)c.Time + c.Duration), "rgba(0, 180, 255, 0.3)", new AgentConnector(target)));
                    }
                    //Riposte
                    ...
                    {
                        int radius = 2200;
                        replay.Decorations.Add(new CircleDecoration(true, 0, radius, ((int)c.Time, (int)c.Time + c.Duration), "rgba(255, 0, 0, 0.5)", new AgentConnector(target)));
                    }
                    //Big Hit
                ...
                case (ushort)WyvernMatriarch:
                    ...
                    {
                        int start = (int)c.Time;
                        int preCast = Math.Min(3500, c.Duration);
                        int duration = Math.Min(6500, c.Duration);
                        Point3D facing = replay.Rotations.LastOrDefault(x => x.Time <= start + 1000);
                        int range = 2800;
                        int span = 2400;
                        if (facing != null)

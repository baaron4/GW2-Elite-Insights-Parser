...
namespace GW2EIEvtcParser.EIData
{
    internal static class ReaperHelper
    {
        internal static readonly List<InstantCastFinder> InstantCastFinder = new List<InstantCastFinder>()
        {
            ...
            new EffectCastFinder(YouAreAllWeaklings, EffectGUIDs.NecromancerYouAreAllWeaklings1).UsingSrcSpecChecker(Spec.Reaper),
            new DamageCastFinder(YouAreAllWeaklings, YouAreAllWeaklings).UsingEnable(combatData => !combatData.HasEffectData),
            new EffectCastFinder(Suffer, EffectGUIDs.NecromancerSuffer).UsingSrcSpecChecker(Spec.Reaper),

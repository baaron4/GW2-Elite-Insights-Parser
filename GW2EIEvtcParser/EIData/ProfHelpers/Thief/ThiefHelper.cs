    ...
    internal static class ThiefHelper
    {
        ...
        internal static void ComputeProfessionCombatReplayActors(AbstractPlayer player, ParsedEvtcLog log, CombatReplay replay)
        {
            ...
            var entranceDecorations = entrances.Select(effect =>
            {
                ...
                GenericAttachedDecoration icon = new IconDecoration(ParserIcons.PortalShadowPortalPrepare, 128, 0.7f, lifespan, connector).UsingSkillMode(player, false);
            ...
            foreach (EffectEvent exit in log.CombatData.GetEffectEventsBySrcWithGUID(player.AgentItem, EffectGUIDs.ThiefShadowPortalActiveExit))
            {
                ...
                replay.Decorations.Add(new CircleDecoration(true, 0, 90, lifespan, color.WithAlpha(0.5f).ToString(), connector).UsingSkillMode(player, false));
                GenericAttachedDecoration icon = new IconDecoration(ParserIcons.PortalShadowPortalOpen, 128, 0.7f, lifespan, connector).UsingSkillMode(player, false);
            ...
            foreach (EffectEvent effect in log.CombatData.GetEffectEventsBySrcWithGUID(player.AgentItem, EffectGUIDs.ThiefSealAreaAoE))
            {
                ...
                replay.Decorations.Add(new IconDecoration("https://wiki.guildwars2.com/images/5/53/Seal_Area.png", 128, 0.5f, lifespan, connector).UsingSkillMode(player, false));
            }
            // Shadow Refuge
            foreach (EffectEvent effect in log.CombatData.GetEffectEventsBySrcWithGUID(player.AgentItem, EffectGUIDs.ThiefShadowRefuge))
            {
                ...
                var connector = new PositionConnector(effect.Position);
                replay.Decorations.Add(new CircleDecoration(false, 0, 240, lifespan, color.WithAlpha(0.5f).ToString(), connector).UsingSkillMode(player, false));
                replay.Decorations.Add(new IconDecoration("https://wiki.guildwars2.com/images/f/f4/Shadow_Refuge.png", 128, 0.5f, lifespan, connector).UsingSkillMode(player, false));
            }
        }
    }
}

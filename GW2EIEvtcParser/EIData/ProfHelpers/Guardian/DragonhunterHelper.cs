...
namespace GW2EIEvtcParser.EIData
{
    internal static class DragonhunterHelper
    {
        ...
        internal static readonly List<DamageModifier> DamageMods = new List<DamageModifier>
        {
            ...
            new BuffDamageModifierTarget(SkillIDs.JusticeDragonhunter, "Big Game Hunter", "10% to tethered target", DamageSource.NoPets, 10.0, DamageType.Strike, DamageType.All, Source.Dragonhunter, ByPresence, "https://wiki.guildwars2.com/images/7/76/Big_Game_Hunter.png", 0, GW2Builds.October2018Balance, DamageModifierMode.All, (x, log) => {
                AgentItem src = x.From;
                AgentItem dst = x.To;
                AbstractBuffEvent effectApply = log.CombatData.GetBuffData(SkillIDs.JusticeDragonhunter).Where(y => y is BuffApplyEvent bae && Math.Abs(bae.AppliedDuration - 8000) < ServerDelayConstant && bae.By == src && bae.To == dst).LastOrDefault(y => y.Time <= x.Time);
                if (effectApply != null)
                {
            ...
            new BuffDamageModifierTarget(SkillIDs.JusticeDragonhunter, "Big Game Hunter", "15% to tethered target", DamageSource.NoPets, 15.0, DamageType.Strike, DamageType.All, Source.Dragonhunter, ByPresence, "https://wiki.guildwars2.com/images/7/76/Big_Game_Hunter.png", GW2Builds.February2020Balance, GW2Builds.EndOfLife, DamageModifierMode.All, (x, log) => {
                AgentItem src = x.From;
                AgentItem dst = x.To;
                AbstractBuffEvent effectApply = log.CombatData.GetBuffData(SkillIDs.JusticeDragonhunter).Where(y => y is BuffApplyEvent bae && Math.Abs(bae.AppliedDuration - 10000) < ServerDelayConstant && bae.By == src && bae.To == dst).LastOrDefault(y => y.Time <= x.Time);
                if (effectApply != null)
                {
        ...
        {
                new Buff("Justice (Dragonhunter)",SkillIDs.JusticeDragonhunter, Source.Dragonhunter, BuffStackType.Stacking, 25, BuffClassification.Other, "https://wiki.guildwars2.com/images/b/b0/Spear_of_Light.png"),

...
internal abstract class SkillMechanic<T> : IDBasedMechanic<T> where T : SkillEvent
{
    ...
    protected bool Minions { get; private set; } = false;
     ...
     public SkillMechanic(long[] mechanicIDs, int id, MechanicPlotlySetting plotlySetting, MechanicDescription description, MechanicSeverity severity, int internalCoolDown, CombatEventsGetter getter) : base(mechanicIDs, id, plotlySetting, description, severity, internalCoolDown)
    {
        _getter = getter;
    }

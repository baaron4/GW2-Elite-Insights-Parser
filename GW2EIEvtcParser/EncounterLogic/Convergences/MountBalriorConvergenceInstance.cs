...
internal class MountBalriorConvergenceInstance : ConvergenceLogic
{
    ...
    internal override List<PhaseData> GetPhases(ParsedEvtcLog log, bool requirePhases)
    {
        ...
        var fullPhase = new PhaseData(Math.Max(log.FightData.FightStart, target.FirstAware), Math.Min(target.LastAware, log.FightData.FightEnd), phaseName, PhaseData.PhaseType.Encounter).WithParentPhase(phases[0]);

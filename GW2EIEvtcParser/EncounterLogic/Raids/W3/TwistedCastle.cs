...
namespace GW2EIEvtcParser.EncounterLogic
{
    internal class TwistedCastle : StrongholdOfTheFaithful
    {
        public TwistedCastle(int triggerID) : base(triggerID)
        {
            ...
            {
                new PlayerBuffApplyMechanic(SpatialDistortion, "Spatial Distortion", new MechanicPlotlySetting("circle",Colors.Magenta), "Statue TP", "Teleported by Statue", "Statue Teleport", 500),
                new PlayerBuffApplyMechanic(StillWaters, "Still Waters", new MechanicPlotlySetting("diamond-tall",Colors.Magenta), "Still Waters (Immunity)", "Used a fountain for immunity", "Still Waters (Immunity)", 0, (evt, log) => log.CombatData.GetBuffData(SoothingWaters).Any(x => x is BuffApplyEvent ba && ba.To == evt.To && Math.Abs(ba.Time - evt.Time) < 500)),
                new PlayerBuffApplyMechanic(StillWaters, "Still Waters", new MechanicPlotlySetting("diamond-tall",Colors.Magenta), "Still Waters (Removal)", "Used a fountain for stack removal", "Still Waters (Removal)", 0, (evt, log) => !log.CombatData.GetBuffData(SoothingWaters).Any(x => x is BuffApplyEvent ba && ba.To == evt.To && Math.Abs(ba.Time - evt.Time) < 500)),
                new PlayerBuffApplyMechanic(Madness, "Madness", new MechanicPlotlySetting("square",Colors.LightPurple), "Madness", "Stacking debuff", "Madness", 0),
                new PlayerBuffApplyMechanic(ChaoticHaze, "Chaotic Haze", new MechanicPlotlySetting("hexagon",Colors.Red), "Chaotic Haze", "Damaging Debuff from bombardement", "Chaotic Haze", 500),
            }
            );
            Extension = "twstcstl";

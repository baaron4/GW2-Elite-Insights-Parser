...
internal class GreerTheBlightbringer : MountBalrior
{
    ...
    private static void AddBlobOfBlight(NPC target, ParsedEvtcLog log, CombatReplay replay)
    {
        ...
        var blobOfBlightMainOrbs = log.CombatData.GetMissileEventsBySkillIDs([BlobOfBlight, BlobOfBlight2, BlobOfBlight3]).Where(x => x.Src.Is(target.AgentItem) && x.LaunchEvents.Any(x => x.Speed == 0.3f));
        var blobOfBlightMiniOrbs = log.CombatData.GetMissileEventsBySkillIDs([BlobOfBlight, BlobOfBlight2, BlobOfBlight3]).Where(x => x.Src.Is(target.AgentItem) && x.LaunchEvents.Any(x => x.Speed == 0.4f || x.Speed == 0.5f));

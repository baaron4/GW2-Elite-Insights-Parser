...
namespace GW2EIEvtcParser.EncounterLogic
{
    internal class MursaatOverseer : BastionOfThePenitent
    {
        public MursaatOverseer(int triggerID) : base(triggerID)
        {
            ...
            new PlayerDstBuffApplyMechanic(Invulnerability757, "Invulnerability", new MechanicPlotlySetting(Symbols.CircleOpen,Colors.Teal), "Protect","Protected by the Protect Shield","Protect Shield",0, (ba, log) => ba.AppliedDuration == 1000),
        ...
        internal override List<PhaseData> GetPhases(ParsedEvtcLog log, bool requirePhases)
        {
            ...
            AbstractSingleActor mainTarget = Targets.FirstOrDefault(x => x.IsSpecy(ArcDPSEnums.TargetID.MursaatOverseer));
            if (mainTarget == null)
            {
        ...
        {
            AbstractSingleActor target = Targets.FirstOrDefault(x => x.IsSpecy(ArcDPSEnums.TargetID.MursaatOverseer));
            if (target == null)
            {
                throw new MissingKeyActorsException("Mursaat Overseer not found");
            }

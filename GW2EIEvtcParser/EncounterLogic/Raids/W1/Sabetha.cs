    ...
    internal class Sabetha : SpiritVale
    {
                ...
                case (int)ArcDPSEnums.TargetID.Sabetha:
                    ...
                    foreach (AbstractCastEvent c in flameWall)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            replay.Decorations.Add(new RectangleDecoration(width, height, (start, start + preCastTime), "rgba(255, 100, 0, 0.2)", positionConnector).UsingRotationConnector(new AngleConnector(facing)));
                            replay.Decorations.Add(new RectangleDecoration(width, height, (start + preCastTime, start + preCastTime + duration), "rgba(255, 50, 0, 0.5)", positionConnector).UsingRotationConnector(new AngleConnector(facing, 360)));
                        }
                    }
                    break;
        ...
        internal override void ComputePlayerCombatReplayActors(AbstractPlayer p, ParsedEvtcLog log, CombatReplay replay)
        {
            ...
            foreach (AbstractBuffEvent c in timedBombs)
            {
                int start = (int)c.Time;
                int end = start + 3000;
                replay.AddDualDecoration(new CircleDecoration(0, 280, (start, end), "rgba(255, 150, 0, 0.5)", new AgentConnector(p)).UsingFilled(false), true, end);
            }
            // Sapper bombs
            ...
            {
                replay.AddDualDecoration(new CircleDecoration(180, seg, "rgba(200, 255, 100, 0.5)", new AgentConnector(p)).UsingFilled(false), true, seg.Start + 5000);

...
internal class Sabetha : SpiritVale
{
    public Sabetha(int triggerID) : base(triggerID)
    {
        ...
        new PlayerCastStartMechanic(KickHeavyBomb, "Kick Heavy Bomb", new MechanicPlotlySetting(Symbols.Cross, Colors.CobaltBlue), "Kick Bomb", "Kicked Heavy Bomb", "Heavy Bomb Kick", 0).UsingChecker((ce, log) => !ce.IsInterrupted && !ce.IsUnknown),

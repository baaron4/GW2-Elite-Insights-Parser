    ...
    internal class Ensolyss : Nightmare
    {
        ...
        private static void AddTormentingBlastDecoration(CombatReplay replay, AbstractSingleActor target, int start, int attackEnd, Point3D point, int quarterAoE, int quarterHit)
        {
            ...
            if (attackEnd >= endQuarter) // If the attack started
            {
                ...
                replay.AddDualDecoration((PieDecoration)new PieDecoration(700, 90, (startQuarter, endQuarter), "rgba(250, 120, 0, 0.2)", connector).UsingRotationConnector(rotationConnector), growingQuarter);
        ...
        private static void AddCausticExplosionDecoration(CombatReplay replay, AbstractSingleActor target, Point3D point, int attackEnd, int start, int end, int growing)
        {
            if (attackEnd >= end) // If the attack started
            {
                ...
                var rotationConnector = new AngleConnector(point);
                // Frontal
                replay.AddDualDecoration((PieDecoration)new PieDecoration(1200, 90, (start, end), "rgba(250, 120, 0, 0.2)", connector).UsingRotationConnector(rotationConnector), growing);
                ...
                replay.AddDualDecoration((PieDecoration)new PieDecoration(1200, 90, (start, end), "rgba(250, 120, 0, 0.2)", connector).UsingRotationConnector(flippedRotationConnector), growing);
                if (end == growing) // If the attack went off
                {
            ...
            switch (target.ID)
            {
                case (int)TargetID.Ensolyss:
                        ...
                        foreach (EffectEvent shieldEffect in shieldEffects)
                        {
                            if (shieldEffect != null)
                            {
                                ...
                                else if (!shield15_0Added)
                                {
                                    ...
                                    replay.AddDualDecoration(new DoughnutDecoration(300, 2000, (start, attackEnd), "rgba(255, 0, 0, 0.2)", new PositionConnector(shieldEffect.Position)), expectedHitEnd, true);
                                }
                            }
                        }
                    }
                    ...
                    foreach (AbstractCastEvent c in causticGrasp)
                    {
                        ...
                        replay.AddDualDecoration(new CircleDecoration(1300, (start, attackEnd), "rgba(250, 120, 0, 0.2)", new AgentConnector(target)), expectedHitEnd);
                    }
                    ...
                    foreach (AbstractCastEvent c in upswingEnso)
                    {
                        ...
                        replay.AddDualDecoration(new CircleDecoration(600, (start, attackEnd), "rgba(250, 120, 0, 0.2)", new AgentConnector(target)), expectedHitEnd);
                        // Shockwave
                    ...
                    foreach (AbstractCastEvent c in causticExplosion)
                    {
                        ...
                        // Circle going in
                        replay.AddDualDecoration(new DoughnutDecoration(0, 2000, (start, attackEnd), "rgba(255, 0, 0, 0.2)", new AgentConnector(target)), expectedHitEnd, true);
                ...
                case (int)TrashID.NightmareHallucination1:
                    ...
                    foreach (AbstractCastEvent c in upswingHallu)
                    {
                        ...
                        replay.AddDualDecoration(new CircleDecoration( 300, (start, endTime), "rgba(250, 120, 0, 0.1)", new AgentConnector(target)), endTime);
                    }
                    break;

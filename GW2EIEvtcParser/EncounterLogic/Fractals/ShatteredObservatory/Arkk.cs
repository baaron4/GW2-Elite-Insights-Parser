...
namespace GW2EIEvtcParser.EncounterLogic
{
    internal class Arkk : ShatteredObservatory
    {
        public Arkk(int triggerID) : base(triggerID)
        {
            ...
            new EnemyCastEndMechanic(39645, "Breakbar End", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "CC.Fail","Breakbar (Failed CC)", "CC Fail",0, (ce,log) => !log.CombatData.GetBuffData(31589).OfType<BuffApplyEvent>().Any(x => x.To == ce.Caster && x.Time > ce.Time && ce.Time < ce.ExpectedEndTime + ParserHelper.ServerDelayConstant)),
            new EnemyCastEndMechanic(39645, "Breakbar End", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "CCed","Breakbar broken", "CCed",0, (ce, log) => log.CombatData.GetBuffData(31589).OfType<BuffApplyEvent>().Any(x => x.To == ce.Caster && x.Time > ce.Time && ce.Time < ce.ExpectedEndTime + ParserHelper.ServerDelayConstant)),

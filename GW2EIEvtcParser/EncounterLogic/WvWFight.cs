    ...
    internal class WvWFight : FightLogic
    {
        ...
        private void SolveWvWPlayers(AgentData agentData, List<CombatItem> combatData, IReadOnlyDictionary<uint, AbstractExtensionHandler> extensions)
        {
            ...
            foreach (AgentItem a in aList)
            {
                ...
                List<AbstractSingleActor> actorListToFill = a.IsNotInSquadFriendlyPlayer ? _friendlyNPCs : _detailed ? _targets : garbageList;
        ...
        internal override void EIEvtcParse(ulong gw2Build, FightData fightData, AgentData agentData, List<CombatItem> combatData, IReadOnlyDictionary<uint, AbstractExtensionHandler> extensions)
        {
            ...
            if (!_detailed)
            {
                var friendlyAgents = new HashSet<AgentItem>(FriendlyNPCs.Select(x => x.AgentItem));

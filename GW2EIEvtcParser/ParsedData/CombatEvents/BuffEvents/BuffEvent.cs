...
public abstract class BuffEvent : TimeCombatEvent
{
    ...
    internal BuffEvent(CombatItem evtcItem, SkillData skillData) : base(evtcItem.Time)
    {
        BuffSkill = skillData.Get(evtcItem.SkillID);
        IFF = evtcItem.IFF;
    }

    internal BuffEvent(SkillItem buffSkill, long time, IFF iff) : base(time)
    {
        BuffSkill = buffSkill;
        IFF = iff;
    }

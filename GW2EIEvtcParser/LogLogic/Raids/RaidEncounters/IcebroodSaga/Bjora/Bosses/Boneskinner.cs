    ...
    public Boneskinner(int triggerID) : base(triggerID)
    {
        MechanicList.Add(new MechanicGroup([
            ...
            new MechanicGroup([
                ...
                new EnemyCastEndMechanic(BoneskinnerCharge, new MechanicPlotlySetting(Symbols.Hexagram, Colors.LightRed), "D.Torch", "Charged a torch", "Charge", MechanicSeverity.P0, 0)
                    .UsingChecker((ce, log) => !ce.IsInterrupted),
            ]),
            new PlayerDstHealthDamageHitMechanic(CrushingCruelty, new MechanicPlotlySetting(Symbols.Star, Colors.DarkGreen), "Crush.Cru.H", "Hit by Crushing Cruelty (Jump middle after Charge)", "Crushing Cruelty Hit", MechanicSeverity.P0, 0),
            new MechanicGroup([
                new PlayerDstHealthDamageHitMechanic(DeathWind, new MechanicPlotlySetting(Symbols.TriangleUp, Colors.Orange), "Launched", "Hit by Death Wind", "Death Wind Hit", MechanicSeverity.P0, 0), // This attack removes stability
                new EnemyCastEndMechanic(DeathWind, new MechanicPlotlySetting(Symbols.TriangleUpOpen, Colors.LightOrange), "D.Wind", "Cast Death Wind (extinguished one torch)", "Death Wind", MechanicSeverity.P0, 0)
                    .UsingChecker((ce, log) => !ce.IsInterrupted),
            ]),
            new MechanicGroup([
                new PlayerDstHealthDamageHitMechanic(DouseInDarkness, new MechanicPlotlySetting(Symbols.Cross, Colors.DarkTeal), "DouseDarkness.H", "Hit by Douse in Darkness", "Douse in Darkness Hit", MechanicSeverity.P1, 0),
                new EnemyCastEndMechanic(DouseInDarkness, new MechanicPlotlySetting(Symbols.Cross, Colors.Teal), "D.Darkness", "Cast Douse in Darkness (extinguished all torches)", "Douse in Darkness", MechanicSeverity.P0, 0)
                    .UsingChecker((ce, log) => !ce.IsInterrupted),
            ]),
            new PlayerDstHealthDamageHitMechanic(BarrageWispBoneskinner, new MechanicPlotlySetting(Symbols.TriangleRight, Colors.Green), "Barrage.H", "Hit by Barrage (Wisp AoE)", "Barrage Hit", MechanicSeverity.P1, 0),
            ...
            new EnemyCastStartMechanic(BoneskinnerBreakbar, new MechanicPlotlySetting(Symbols.Square, Colors.Purple), "Breakbar", "Casting a Breakbar", "Breakbar", MechanicSeverity.P1, 0),
            new EnemyDstBuffApplyMechanic(Exposed31589, new MechanicPlotlySetting(Symbols.SquareOpen, Colors.Pink), "Exposed.E", "Gained Exposed (Breakbar broken)", "Exposed", MechanicSeverity.P1, 0),
        ])
        );
        Extension = "boneskin";

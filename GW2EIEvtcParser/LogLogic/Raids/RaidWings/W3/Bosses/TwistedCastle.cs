...
internal class TwistedCastle : StrongholdOfTheFaithful
{
    internal readonly MechanicGroup Mechanics = new([
            ...
            new MechanicGroup([
                new PlayerDstBuffApplyMechanic(StillWaters, new (Symbols.DiamondTall,Colors.Magenta), "Still Waters (Immunity)", "Used a fountain for immunity", "Still Waters (Immunity)", Sev2, 0)
                ...
                new PlayerDstBuffApplyMechanic(StillWaters, new (Symbols.StarDiamond,Colors.Magenta), "Still Waters (Removal)", "Used a fountain for stack removal", "Still Waters (Removal)", Sev2, 0)
                    .UsingChecker((evt, log) => !log.CombatData.GetBuffDataByIDByDst(SoothingWaters, evt.To).Any(x => x is BuffApplyEvent ba && Math.Abs(ba.Time - evt.Time) < 500)),
            ]),
            new PlayerDstBuffApplyMechanic(Madness, new (Symbols.Square,Colors.LightPurple), "Madness", "Stacking debuff", "Madness", Sev1, 0),

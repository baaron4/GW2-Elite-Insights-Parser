...
namespace GW2EIBuilders.HtmlModels
{
    internal class CombatReplayDto
    {
        ...
        private static List<object> GetCombatReplayActors(ParsedEvtcLog log, CombatReplayMap map, Dictionary<long, SkillItem> usedSkills, Dictionary<long, Buff> usedBuffs)
        {
            ...
            {
                if (actor.IsFakeActor || actor.GetCombatReplayNonPolledPositions(log).Count == 0)
                {
                    continue;
                }
            ...
            {
                if ((actor.LastAware - actor.FirstAware < 200) || actor.GetCombatReplayNonPolledPositions(log).Count == 0)
                {
                    continue;
                }
                actors.Add(actor.GetCombatReplayDescription(map, log));
